...
# Starts a scenario with a repository at qBittorrent 5.2.1 which has already
# seen two releases of it (v5.2.1-0 and v5.2.1-1).
scenario() {
	echo "$1"
...
expect 'task edit'    v5.2.1-2 "$(merge "$edit_task")"
expect 'version bump' v5.3.0-0 "$(merge "$bump_version")"

scenario 'Commits that do not affect the role'
expect 'README'   ''        "$(merge "$edit_readme")"
expect 'a script' ''        "$(merge "$edit_script")"
expect 'a task'   v5.2.1-2 "$(merge "$edit_task")"

scenario 'Release numbers past 9'
for release_number in 2 3 4 5 6 7 8 9 10; do
	git tag "v5.2.1-$release_number"
done
expect 'a task' v5.2.1-11 "$(merge "$edit_task")"

scenario 'Reverting to an already released version'
merge "$bump_version" > /dev/null
# The role is now identical to what v5.2.1-1 already published, so there is
# nothing new to release.
expect 'a revert' ''        "$(merge "$revert_version")"

scenario 'Reverting to an already released version, with a change'
merge "$bump_version" > /dev/null
expect 'a revert' v5.2.1-2 "$(merge "$revert_version && $edit_task")"
